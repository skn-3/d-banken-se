...
function ActivatePage() {
  ...
  if (hasSession && rawCode && team) {
    return <Shell wide>{joining ? <p className="text-center text-sm" style={{ color: "var(--muted-foreground)" }}>Kopplar dig till laget…</p> : <IntroCarousel team={team} onDone={() => navigate({ to: "/saljare", search: {} })} />}</Shell>;
  }
...
function InvitePasswordCard() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setDone(true);
    setTimeout(() => navigate({ to: "/saljare", search: {} }), 1200);
  };
